...
from src.services import input_user_month, input_user_year, sort_operations_by_user_month_year, sort_data_by_categories


# Тесты для input_user_month
@patch('builtins.input')
def test_input_user_month_correct_first_try(mock_input):
...
@patch('builtins.input')
def test_input_user_month_correct_after_invalid_string(mock_input):
...
@patch('builtins.input')
def test_input_user_month_correct_after_out_of_range(mock_input):
...
@patch('builtins.input')
def test_input_user_month_empty_then_correct(mock_input):
...
@patch('builtins.input')
def test_input_user_month_multiple_empty_then_correct(mock_input):
    ...
    assert calls[0] == call("Введите месяц за который необходим анализ\n"
                            "(от 1 до 12, где 1-январь...12-декабрь): ")
    for i in range(1, 4):
        assert calls[i] == call("Не верные данные ввода месяца, попробуйте еще раз\n"
                                "(от 1 до 12, где 1-январь...12-декабрь): ")


@patch('builtins.input')
def test_input_user_month_never_correct(mock_input):
    ...
    for i, call_args in enumerate(mock_input.call_args_list):
        if i == 0:
            ...
            expected = call("Не верные данные ввода месяца, попробуйте еще раз\n"
                            "(от 1 до 12, где 1-январь...12-декабрь): ")
...
@patch('builtins.input')
def test_input_user_year_correct_first_try(mock_input):
    """Пользователь сразу вводит корректный год."""
    mock_input.return_value = "2020"
    result = input_user_year()
    assert result == 2020
    mock_input.assert_called_once_with(
        "Введите год за который необходим анализ (2018 до 2021): "
    )


@patch('builtins.input')
def test_input_user_year_correct_after_invalid_string(mock_input):
...
@patch('builtins.input')
def test_input_user_year_correct_after_out_of_range(mock_input):
...
@patch('builtins.input')
def test_input_user_year_empty_then_correct(mock_input):
...
@patch('builtins.input')
def test_input_user_year_multiple_invalid_then_correct(mock_input):
...
@patch('builtins.input')
def test_input_user_year_never_correct(mock_input):
...
def test_sort_operations_by_user_month_year_valid(mock_year, mock_month):
...
@patch('src.services.input_user_month')
@patch('src.services.input_user_year')
def test_sort_operations_by_user_month_year_no_matches(mock_year, mock_month):
...
@patch('src.services.input_user_month')
@patch('src.services.input_user_year')
def test_sort_operations_by_user_month_year_invalid_dates(mock_year, mock_month):
...
@patch('src.services.input_user_month')
@patch('src.services.input_user_year')
def test_sort_operations_by_user_month_year_empty_df(mock_year, mock_month):
...
@patch('src.services.input_user_month')
@patch('src.services.input_user_year')
def test_sort_operations_by_user_month_year_month_year_order(mock_year, mock_month):
...
def test_sort_data_by_categories_success(mock_read_excel, mock_sort):
...
@patch('src.services.sort_operations_by_user_month_year')
@patch('src.services.read_excel_file')
def test_sort_data_by_categories_no_excluded_categories(mock_read_excel, mock_sort):
    """Проверка, что если нет категорий из списка исключения, возвращаются все."""
    data = {
        "Категория": ["Такси", "Интернет", "Телефон"],
        "Сумма платежа": [500.00, 300.50, 150.25]
    }
    filtered_df = pd.DataFrame(data)
...
@patch('src.services.sort_operations_by_user_month_year')
@patch('src.services.read_excel_file')
def test_sort_data_by_categories_empty_after_filter(mock_read_excel, mock_sort):
...
@patch('src.services.sort_operations_by_user_month_year')
@patch('src.services.read_excel_file')
def test_sort_data_by_categories_all_excluded(mock_read_excel, mock_sort):
...
@patch('src.services.sort_operations_by_user_month_year')
@patch('src.services.read_excel_file')
def test_sort_data_by_categories_rounding(mock_read_excel, mock_sort):
    """Проверка округления сумм до двух знаков."""
    data = {
        "Категория": ["Продукты", "Одежда"],
        "Сумма платежа": [100.555, 200.999]
    }
    filtered_df = pd.DataFrame(data)
    mock_read_excel.return_value = pd.DataFrame()
    mock_sort.return_value = filtered_df
